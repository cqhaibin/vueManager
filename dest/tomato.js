...
var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if("value"in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};}();function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}var Service=function(){function Service(store,storage){_classCallCheck(this,Service);this.$store=store;this.storage=storage;this.storageKey="RECORDS";this.currentRecord=null;this._prefix="record_";this.$state=this.$store.state;this.loadData();}_createClass(Service,[{key:'addRecord',value:function addRecord(){this.$store.commit(keys.addRecord,this.currentRecord);this.storage.changeValue(this.storageKey,JSON.stringify(this.$state.tomato.records));}},{key:'getRecords',value:function getRecords(){return this.$state.tomato.records;}},{key:'clear',value:function clear(){this.$store.commit(keys.removeAllRecord);this.storage.changeValue(this.storageKey,"");}},{key:'loadData',value:function loadData(){var _this=this;var str=this.storage.readValue(this.storageKey);if(str){var arrayData=JSON.parse(str);arrayData.forEach(function(data,index){_this.$store.commit(keys.addRecord,data);});}}/**
     * 开始做事情
     * @param type TYPE，事件状态
     * @param duration 时长，秒
     * @param content 内容
     */},{key:'start',value:function start(type,duration,content){var record=new Record(),nowDate=new Date();record.id=uniqueId_1(this._prefix);record.begTime=nowDate;record.endTime=new Date(nowDate.getTime()+duration*1000);record.type=type;record.content=content;this.currentRecord=record;}/**
     * 停止做事情 
     * @param isauto 是否自动停止
